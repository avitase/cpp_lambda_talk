...
\begin{frame}{Let's talk about Python}
    \centering
    \scalebox{4.}{%
        \only<1>{\texttt{Rage mode: off}}%
        \only<2>{\texttt{Rage mode: on}}%
    }
\end{frame}

\begin{frame}[fragile]{What is the output of the program?}
...
print(f()[1](2))
    ...
    \only<2,3>{\textbf{Answer:} \texttt{4} (not \texttt{3})\ldots}

    \only<3>{\hfill \textbf{Why?} implicit capture by reference}
\end{frame}
...
\begin{frame}[plain]
    \centering
    \scalebox{4.}{%
        \only<1>{\texttt{Rage mode: on}}%
        \only<2>{\texttt{Rage mode: off}}%
    }
\end{frame}
